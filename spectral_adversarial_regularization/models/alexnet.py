...
def alexnet(input_data, num_classes=10, wd=0, update_collection=None, beta=1., reuse=None):
...
def alexnet_nolrn(input_data, num_classes=10, wd=0, update_collection=None, beta=1., reuse=None):
    """AlexNet architecture
        two [convolution 5x5 -> max-pool 3x3 -> local-response-normalization] modules 
        followed by two fully connected layers with 384 and 192 hidden units, respectively. 
        Finally a NUM_CLASSES-way linear layer is used for prediction
...
def alexnet_sn(input_data, num_classes=10, wd=0, update_collection=None, beta=1., reuse=None):
