...
def alexnet(input_data, NUM_CLASSES, wd=0, update_collection=None, beta=1.):
...
def alexnet_nolrn(input_data, NUM_CLASSES, wd=0, update_collection=None, beta=1.):
    """AlexNet architecture
        two [convolution 5x5 -> max-pool 3x3 -> local-response-normalization] modules 
        followed by two fully connected layers with 384 and 192 hidden units, respectively. 
        Finally a NUM_CLASSES-way linear layer is used for prediction
    ...
    conv = sn.conv2d(pool1, [5, 5, 96, 256], scope_name='conv2', spectral_norm=False)
    conv2 = tf.nn.relu(conv, name='conv2_relu')
    pool2 = tf.nn.max_pool(conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool2')
...
def alexnet_sn(input_data, NUM_CLASSES, wd=0, update_collection=None, beta=1.):
    ...
    conv = sn.conv2d(input_data, [5, 5, 3, 96], scope_name='conv1', update_collection=update_collection, beta=beta)
    conv1 = tf.nn.relu(conv, name='conv1_relu')
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool1')
    norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm1')
