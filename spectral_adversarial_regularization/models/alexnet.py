...
def alexnet(NUM_CLASSES, wd=0, update_collection=None):
...
def alexnet_sn(NUM_CLASSES, wd=0, update_collection=None):
    ...
    conv = sn.conv2d(input_data, [5, 5, 3, 96], scope_name='conv1', update_collection=update_collection)
    conv1 = tf.nn.relu(conv, name='conv1_relu')
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool1')

    conv = sn.conv2d(pool1, [5, 5, 96, 256], scope_name='conv2', update_collection=update_collection)
    conv2 = tf.nn.relu(conv, name='conv2_relu')
    pool2 = tf.nn.max_pool(conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool2')
    
    reshape = tf.reshape(pool2, [-1, 6*6*256])
    lin = sn.linear(reshape, 384, scope_name='linear1', update_collection=update_collection)
    lin1 = tf.nn.relu(lin, name='linear1_relu')

    lin = sn.linear(lin1, 192, scope_name='linear2', update_collection=update_collection)
    lin2 = tf.nn.relu(lin, name='linear2_relu')

    fc = sn.linear(lin2, NUM_CLASSES, scope_name='fc', update_collection=update_collection)
        
    return input_data, input_labels, fc


def alexnet_sar(NUM_CLASSES, wd=0, update_collection=None):
