...
def alexnet_sn(NUM_CLASSES, wd=0, update_collection=None):
    ...
    conv = sn.conv2d(input_data, [5, 5, 3, 64], scope_name='conv1', update_collection=update_collection)
    conv1 = tf.nn.relu(conv, name='conv1_relu')
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool1')

    conv = sn.conv2d(pool1, [5, 5, 64, 64], scope_name='conv2', update_collection=update_collection)
    conv2 = tf.nn.relu(conv, name='conv2_relu')
    pool2 = tf.nn.max_pool(conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool2')
