...
def incept(input_x, input_filters, ch1_filters, ch3_filters, spectral_norm=True,
           scope_name='incept', update_collection=None, bn=None):
    """Inception module"""
...
def downsample(input_x, input_filters, ch3_filters, spectral_norm=True,
               scope_name='downsamp', update_collection=None, bn=None):
    """Downsample module"""
...
def inception(NUM_CLASSES, wd=0, update_collection=None):
...
def inception_nobn(NUM_CLASSES, wd=0, update_collection=None):
...
def inception_sn(NUM_CLASSES, wd=0, update_collection=None):
    ...
    layer1 = tf.nn.relu(sn.conv2d(input_data, [3, 3, 3, 96], scope_name='conv1', update_collection=update_collection))
    layer2 = incept(layer1, 96, 32, 32, scope_name='incept2', update_collection=update_collection)
    layer3 = incept(layer2, 32+32, 32, 48, scope_name='incept3', update_collection=update_collection)
    layer4 = downsample(layer3, 32+48, 80, scope_name='downsamp4', update_collection=update_collection)
    layer5 = incept(layer4, 80+32+48, 112, 48, scope_name='incept5', update_collection=update_collection)
    layer6 = incept(layer5, 112+48, 96, 64, scope_name='incept6', update_collection=update_collection)
    layer7 = incept(layer6, 96+64, 80, 80, scope_name='incept7', update_collection=update_collection)
    layer8 = incept(layer7, 80+80, 48, 96, scope_name='incept8', update_collection=update_collection)
    layer9 = downsample(layer8, 48+96, 96, scope_name='downsamp9', update_collection=update_collection)
    layer10 = incept(layer9, 96+48+96, 176, 160, scope_name='incept10', update_collection=update_collection)
    layer11 = incept(layer10, 176+160, 176, 160, scope_name='incept11', update_collection=update_collection)
    layer12 = tf.nn.pool(layer11, window_shape=[7, 7], pooling_type='AVG',
                         padding='SAME', strides=[1, 1], name='mean_pool12')
    
    fc = sn.linear(layer12, NUM_CLASSES, scope_name='fc', xavier=True, update_collection=update_collection)
