...
def alexnet_sn(NUM_CLASSES, wd=0, update_collection=None):
    ...
    conv = sn.conv2d(input_data, [5, 5, 3, 96], scope_name='conv1', tighter_sn=True, update_collection=update_collection)
    conv1 = tf.nn.relu(conv, name='conv1_relu')
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                           padding='VALID', name='pool1')

    conv = sn.conv2d(pool1, [5, 5, 96, 256], scope_name='conv2', tighter_sn=True, update_collection=update_collection)
